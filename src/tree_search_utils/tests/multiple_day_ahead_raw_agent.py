...
def solve_day_ahead(env, 
    ...
    for t in range(env.episode_length):
        ...
        #fidn the best action accordring to the policy
        a_best, sub_obs, sub_acts, log_probs = policy.generate_action(env, obs = env.state)
        
        
        final_schedule[t, :] = a_best
        
        obs, reward, done, _ = env.step(a_best, deterministic=True)

        print(f"Period {env.episode_timestep+1}", np.array(a_best, dtype=int), round(reward, 2), round(time.time()-period_start_time, 2))
