...
def run_epoch(
    ...
    while epoch_done is False:
        ...
        for idx in range(len(sub_obs)):
            local_ac.actor_buffer.store(
                sub_obs[idx].cpu(), sub_acts[idx].cpu(), log_probs[idx].cpu(), reward, value
            )

        obs = new_obs

        if done:
            local_ac.actor_buffer.finish_ep_new(
                ts=ep_sub_acts, ep_rews=ep_rews, ep_vals=ep_vals, last_val=0
            )
        ...
        if local_ac.actor_buffer.is_full() or sum(timesteps) > steps_per_epoch:
            if not done:
                local_ac.actor_buffer.finish_ep_new(
                    ts=ep_sub_acts,
                    ep_rews=ep_rews,
                ...
                local_ac.critic_buffer.finish_ep(
                    last_val=local_ac.get_value(obs)[0].detach().cpu().numpy()
                )

            entropy, loss_v, explained_variance = shared_ac.update(
                local_ac, pi_optimizer, v_optimizer
            )
            mean_entropy, loss_v, explained_variance = (
            ...
            # print statements
            print(
                f"Finished epoch {epoch_counter.item()} with mean reward {np.mean(all_ep_rewards)} and mean timesteps {np.mean(timesteps)}, Total ts: {sum(timesteps)}")
            epoch_done = True
...
def run_worker(
    ...
    while epoch_counter < num_epochs:
        ...
        if params.get("entropy_target", 0) != 0:
            shared_ac.entropy_coef = params.get("entropy_coef") * (
                3 * epoch_counter.item() / num_epochs
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--observation_processor", type=str, required=False, default="LimitedHorizonProcessor"
    )
    ...
    num_epochs = int(args.timesteps / args.steps_per_epoch)
    ...
    params.update(
        {"env_fn": f"src/rl4uc/data/envs/{args.num_gen}gen.json", "env_name": f"{args.num_gen}gen"}
    )
