...
if [ -z "$3" ]
then
    echo $0: no argument for action method, defaulting to max_min. Values are supposed to be in {max_min, max_avg, max_min_central}
...
if [ -z "$4" ]
then
    echo $0: no argument for obs corrupter, defaulting to auto. Values are supposed to be in {auto, box}
    obs_corrupter=auto
else    
    obs_corrupter=$4
fi
...
if [ -z "$5" ]
then
    echo $0: no argument for num trees, defaulting to 10
    num_trees=10
else    
    num_trees=$5
fi
...
save_dir=results/final_results/guided_forest/${num_trees}/${agent}/${num_gen}gen/action_${action_method}/obs_${obs_corrupter}/test_${num_files}/$seed
